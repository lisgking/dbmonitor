...
* Modify: qmw 2014-5-19 扩展了复选框disabled, 
*         cellFormatter扩展了第二个参数，rowData行数据,
*         root元素扩展了updateGridLayout事件监听(用于重绘grid)
*         扩展afterSelectionChange无论如何都返回true
...
// kg.defaultRowTemplate = function(){ return '<div data-bind="attr:{title: $grid.setTitle($data.entity)}, foreach: $grid.visibleColumns, css: { \'ui-widget-content\': $grid.jqueryUITheme }"><div data-bind="attr: { \'class\': cellClass() + \' kgCell col\' + $index() }, kgCell: $data"></div></div>';};
...
ko.bindingHandlers['koGrid'] = (function () {
    return {
        'init': function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            $(elem).on('updateGridLayout', function(){
...
kg.Column = function (config, grid) {
    ...
    self.getProperty = function (row) {
        ...
        if (self.cellFilter) {
            ...
            //qmw 2014-5-19扩展cellFormatter参数当前行data
            ret = self.cellFilter(row.getProperty(self.field), row.entity);
        } else {
...
kg.RowFactory = function(grid) {
    ...
    self.filteredDataChanged = function() {
        ...
        //add by lisg start 2015-3-27
        //self.selectionService.toggleSelectAll(false);
        self.selectionService.toggleSelectAll(grid.allSelected());
        //add by lisg end 2015-3-27
...
kg.Grid = function (options) {
    ...
    //qmw 2014-5-19扩展afterSelectionChange无论如何都返回true
    var _afterSelectionChange = self.config.afterSelectionChange;
    self.config.afterSelectionChange = function(){
        _afterSelectionChange.apply(this, arguments);
        return true;
    }
    ...
    self.buildColumns = function () {
        ...
        if (self.config.displaySelectionCheckbox) {
            columnDefs.splice(0, 0, {
                ...
                //qmw 2014-5-19扩展disabled
                cellTemplate: '<div class="kgSelectionCell"><input class="kgSelectionCheckbox" type="checkbox" data-bind="checked: $parent.selected, attr:{disabled: $parent.disabledHandle($root)}" /></div>'
    ...
    self.setTitle = function(itemData){
        // console.log(itemData());
        return this.config.setTitle && this.config.setTitle(itemData) || '';
    };
    ...
    self.totalPages = self.config.pagingOptions.totalPages;
    ...
    self.maxPages = ko.computed(function () {
        ...
        if(self.config.pagingOptions.totalPages){
            return self.config.pagingOptions.totalPages();
        }
...
kg.Row = function (entity, config, selectionService) {
    ...
    self.selected = ko.observable(false);
     //qmw 2014-5-19扩展disabled
    self.disabledHandle = function(rowVM){
        if(rowVM.$grid.config.beforeDisabledCB){
            return rowVM.$grid.config.beforeDisabledCB(rowVM.entity);
        }
        return false;
    }
    ...
    self.toggleSelected = function (row, event) {
        //qmw 2014-5-19扩展disabled
        if(row.$grid.config.beforeDisabledCB && row.$grid.config.beforeDisabledCB(row.entity)){
            return ;
        }
        ...
        if (config.selectWithCheckboxOnly && element.type != "checkbox"){
            ...
            if (self.beforeSelectionChange(self, event)) {
                ...
                flag = self.afterSelectionChange(self, event);
                // return self.afterSelectionChange(self, event);
            }
        }
        //qmw 2014-7-30扩展“全选”框根据实际情况自动改变勾选状态
        if(row.$grid.$userViewModel.myData().length !== row.$grid.selectedItems().length){
            row.$grid.allSelected(false);
        }else{
            row.$grid.allSelected(true); 
        }
        return flag;
	...
	self.ChangeSelection = function (rowItem, evt) {
	    ...
	    if (!self.multi) {
	        if (self.lastClickedRow && self.lastClickedRow.selected) {
                //2014-7-15添加单选时也可以取消选择
                oldSelected = self.lastClickedRow.selected();
	    ...
	    if (grid.config.keepLastSelected && !self.multi) {
            ...
            //2014-7-15添加单选时也可以取消选择
            if(typeof oldSelected === 'boolean' && rowItem == self.lastClickedRow){
                self.setSelection(self.lastClickedRow, !oldSelected);
            }
	    } else {
    ...
    self.toggleSelectAll = function (checkAll) {
        ...
        $.each(self.rowFactory.rowCache, function (i, row) {
            ...
            if (row && row.selected) {
                ...
                //qmw 2014-7-1扩展点全选时 __kg_selected__值没有改变
                row.entity[SELECTED_PROP] = checkAll;
            }
        });
        return true;
    };
...
kg.domUtilityService = {
    ...
    UpdateGridLayout: function(grid) {
        ...
        if (grid.$root.parents(":hidden").length > 0 && grid.$root.width() < 0) {
            return;
        }
