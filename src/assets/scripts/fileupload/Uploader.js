...
define([
	...
	"ts/util/Format",
	"dojo/text!./htm/Uploader.htm",
	"dojo/css!./css/Uploader.css",
	"dojo/nls!./nls/Uploader.json"
],function(MessageBundle,HTMLBundleMap,Format,htm,css,json){
	var i18n=new MessageBundle(json);
	var __msg__=i18n.__data__;
	var htmlBundle=new HTMLBundleMap(htm);
	window.htmlBundle=htmlBundle;
	...
	var Provider, aFilters = [], nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sCellFileTemplate = htmlBundle.getMessage("sCellFileTemplate",__msg__).trim(),
		sTotalContainer = htmlBundle.getMessage("sTotalContainer",__msg__).trim(),
		sUploadStartError = htmlBundle.getMessage("sUploadStartError",__msg__).trim(),
		sFilesContainer = htmlBundle.getMessage("sFilesContainer",__msg__).trim();
	...
	function fShowMessage0(msg, warning) {
	...
	function fShowMessage(msg, warning) {
		if(warning){
			Dialog.alert('<div class="alert alert-danger">'+msg+'</div>');
		}else{
			fShowMessage0(msg,warning);
		}
	}
	...
	StreamProvider.prototype = {
		...
		traverseFileTree : function (directory, callback) {
			...
			reader.readEntries(function(entries) {
				callback.pending--;
				if (!entries.length) {
					fShowMessage(i18n.getMessage("emptyDirectoryIgnored",[relativePath+directory.name]), true);
				} else {
	...
	function Main(cfg){
		...
		this.config = {
			...
			dragAndDropTips: cfg.dragAndDropTips || '<div><span>'+i18n.getMessage("chooseFile")+'</span><span>'+ i18n.getMessage("dropFileHere")+'</span></div>',
			fileFieldName : 'FileData',
			browseFileId : cfg.browseFileId || 'i_select_files',
			browseFileBtn : cfg.browseFileBtn || '<button type="button" title="'+i18n.getMessage("chooseFile")+'"></button>',
			...
			onFileNameMisMatch : cfg.onFileNameMisMatch,
	...
	Main.prototype = {
		...
		onSelect : function(list) {
			fShowMessage(i18n.getMessage("onSelect",list));
		},
		onFileCountExceed : function(selected, limit) {
			fShowMessage(i18n.getMessage("onFileCountExceed",{selected:selected,limited:limit}), true);
		},
		onMaxSizeExceed : function(size, limited, name) {
			fShowMessage(i18n.getMessage("onFileCountExceed",{size:size,limited:limited,name:name}), true);
		},
		onExtNameMismatch: function(name, filters) {
			fShowMessage(i18n.getMessage("onExtNameMismatch",{filters:"*"+filters.join(", *"),name:name}), true);
		},
		onCancel : function(info) {
			fShowMessage(i18n.getMessage("onCancel",info));
		},
		onStop : function() {
			fShowMessage(i18n.getMessage("onStop"));
		},
		onCancelAll : function(numbers) {
			fShowMessage(i18n.getMessage("onCancelAll",[numbers]));
		},
		onComplete : function(info) {
			fShowMessage(i18n.getMessage("onComplete",info));
		},
		onQueueComplete : function(info) {
			fShowMessage(i18n.getMessage("onQueueComplete",info));
		},
		onUploadError : function(status, msg) {
			fShowMessage(i18n.getMessage("onUploadError",{status:status,msg:msg}), true);
		},
		disable : function() {
		...
		reupload: function(){
			this.getNode("_stream-total-size", this.totalContainerPanel).innerHTML = '';
			this.getNode("_stream-total-uploaded", this.totalContainerPanel).innerHTML = '';
			this.getNode("stream-percent", this.totalContainerPanel).innerHTML = "0%";
			this.getNode("stream-process-bar", this.totalContainerPanel).innerHTML = '<span style="width: 0%;"></span>';
			this.getNode("stream-cell-file").remove();
		...
		unloadHandler : function(evt) {
			...
			if (this.waiting.length > 0)
				return evt.returnValue = i18n.getMessage("fileSizeOverflowInFlash");
		},
		upload : function(index) {
			...
			xhr.onreadystatechange = function() {
				...
				try {
					...
					if (token) {
						...
						var msg=i18n.getMessage("failedToCreateTask",[self.get("tokenURL"),xhr.status]);
			...
			xhr.onerror = function() {
				self.cancelOne(index);
				var msg=i18n.getMessage("errorToCreateTask",[xhr.status]);
				fShowMessage(msg, true);
			};
			xhr.send();
		},
		...
		fileSelect : function(a) {
			...
			if (b == this.get("simLimit") || a.length > this.get("simLimit")) {
				...
				fShowMessage(i18n.getMessage("fileNumOverflowInTask",{num:this.get("simLimit"),length:a.length}), true);
				return !1;
			}
			var isShow=false;
			for (c = 0; c < a.length; c++){
				if(this.validateFile(a[c])){
					if(!isShow){
						this.get("onSelect") ? this.get("onSelect")(a) : this.onSelect(a);
						isShow=true;
					}
					this.addStreamTask(a[c]);
				}
			}
		},
		validateFile : function(uploader) {
			...
			if(!bStreaming && size > 2147483648){fShowMessage(i18n.getMessage("fileSizeOverflowInFlash",{name:name,size:size}), true);return !1;}
			...
			else if(this.get("onFileNameMisMatch")){
				valid=this.get("onFileNameMisMatch")(name, fileNameReg);
				if(!valid){
					return valid;
				}
			}else {
		...
		formatBytes:function(length){
			return Format.toSizeString(length,2).replace(" ","");
		},
	...
	/** window.Uploader is a reserved word for SWFUpload Component */
